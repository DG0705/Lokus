...
import { Package, MapPin, QrCode, Smartphone, Banknote, X, CheckCircle, ShoppingBag } from 'lucide-react';

export default function StaticCheckoutPage() {
  const { id } = useParams(); // The string ID from the static DB
  ...
  const MERCHANT_UPI_ID = "YOUR_PERSONAL_UPI_ID@bank"; 
  ...
  useEffect(() => {
    ...
    const actualUserId = token ? parseInt(token.replace('mock_token_', '')) || 1 : 1;

    // Fetch the Static Shoe details
    fetch(`http://127.0.0.1:8000/api/v1/catalog/${id}?user_id=${actualUserId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const executeFinalCheckout = async () => {
    ...
    try {
      ...
      const actualUserId = parseInt(getCookie('token')?.replace('mock_token_', '') || '1');
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/v1/checkout/static/${id}`, {
        method: 'POST',
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-8 font-sans">
      <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-xl text-center border border-red-100">
        <h2 className="text-2xl font-black text-red-600 uppercase tracking-tighter mb-4">Checkout Error</h2>
        <p className="text-gray-500 font-medium mb-8">{error}</p>
        <Link href="/collection" className="block w-full bg-black text-white font-bold uppercase px-8 py-4 rounded-xl hover:bg-gray-800 transition-all">Back to Archive</Link>
      </div>
    </div>
  );

  if (!data) return <div className="min-h-screen flex justify-center items-center bg-white uppercase font-black tracking-widest text-gray-400">Securing Details...</div>;
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-900 p-4 md:p-8 font-sans relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              ...
              <button 
                onClick={executeFinalCheckout} disabled={isProcessing || !isPaymentValid}
      ...
      <div className={`max-w-4xl mx-auto mt-12 transition-all ${showPaymentModal ? 'blur-sm opacity-50 select-none' : ''}`}>
        ...
        {/* RELAXED HEADER (No Timer) */}
        <div className="bg-black text-white p-6 rounded-3xl flex justify-between items-center mb-8 shadow-xl shadow-black/10">
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400">Standard Checkout</h2>
            <p className="text-xl md:text-2xl font-black uppercase tracking-tighter">Complete Purchase</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-200">
            ...
            <div className="border-t border-gray-100 pt-6">
               ...
               <div className="flex justify-between items-center mb-4"><span className="text-gray-500 font-semibold">Standard Shipping</span><span className="font-bold text-green-600">Free</span></div>
