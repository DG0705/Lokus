...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const distance = new Date(data.reservation.expires_at).getTime() - new Date().getTime();
      
      if (distance <= 0) {
        clearInterval(interval);
        setError("Time Expired. Your pair was released to the pool.");
        return;
      }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Success! Send them to the Vault to see their new shoe
      router.push('/vault');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-100 p-8">
        <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-xl text-center border-2 border-red-100">
          ...
          <Link href="/" className="bg-stone-900 text-white font-bold uppercase px-8 py-4 rounded-xl hover:bg-stone-800">
            Back to Drops
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-red-800 text-white p-6 rounded-3xl flex justify-between items-center mb-8 shadow-lg shadow-red-900/20">
          <div>
            ...
            <p className="text-xl font-black uppercase tracking-tighter">Complete Checkout</p>
          </div>
          <div className="text-right">
            <h2 className="text-xs font-bold uppercase tracking-widest text-red-300">Releasing In</h2>
            <p className="text-3xl font-mono font-light">{timeLeft || "00:00"}</p>
          </div>
        </div>
                ...
                <button 
                  ...
                  className={`py-4 rounded-xl font-bold uppercase transition-all ${
                    selectedSize === size 
                      ...
                      : 'bg-neutral-100 text-stone-500 hover:bg-neutral-200'
                  }`}
                >
                  {size}
                </button>
